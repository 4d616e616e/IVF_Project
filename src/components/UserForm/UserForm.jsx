  ...
  const validateStep = (step) => {
    ...
    switch (step) {
      case 1: // Trying Duration
        if (formData.tryingDuration === 0) {
          newErrors.tryingDuration =
            "Please select how long you have been trying";
        }
        break;
      case 2: // Working Status
        if (!formData.workingCouple) {
          newErrors.workingCouple = "Please select an option";
        }
        if (
          formData.workingCoupleDescription &&
          formData.workingCoupleDescription.length > 200
        ) {
      ...
      case 3: // Medical History
        if (!formData.medicalHistory) {
          newErrors.medicalHistory = "Please select an option";
        }
        if (
          formData.medicalHistoryDescription &&
          formData.medicalHistoryDescription.length > 200
        ) {
      ...
      case 4: // Health Issues
        if (!formData.healthIssues) {
          newErrors.healthIssues = "Please select an option";
        }
        if (
          formData.healthIssuesDescription &&
          formData.healthIssuesDescription.length > 200
        ) {
        ...
        break;
      case 5: // Sperm Test
        if (!formData.spermTest) {
          newErrors.spermTest = "Please select an option";
        }
        if (
          formData.spermTestDescription &&
          formData.spermTestDescription.length > 200
        ) {
      ...
      case 6: // Family History
        ...
        if (
          ...
          formData.familyHistoryDescription.length > 200
        ) {
    ...
    try {
      const response = await fetch(
        `https://devapi.homeivf.com/api/form/${form_id}`,
        {
          ...
          body: JSON.stringify({
            ...
            any_past_treatment:
              formData?.medicalHistory === "yes" ? true : false,
      ...
      if (response.ok && result.status) {
        setAlert({
          open: true,
          message:
            "Form submitted successfully! Our expert will contact you soon.",
          severity: "success",
        });
          ...
          <Box sx={{ py: 4 }}>
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 2 }}>
              ...
              >
                ...
                {errors.tryingDuration && (
                  <FormHelperText>{errors.tryingDuration}</FormHelperText>
                )}
              </FormControl>
            ...
            {formData.spermTest === "yes" && (
              <TextField
                ...
                helperText={
                  ...
                  `${formData.spermTestDescription?.length || 0}/200 characters`
                }
                sx={{
                  mt: 2,
                  "& .MuiOutlinedInput-root": {
